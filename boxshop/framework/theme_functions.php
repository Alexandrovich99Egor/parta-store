			...
			<a href="<?php echo esc_url(home_url('/')); ?>">
				...
				<?php if ($logo_image): ?>
					<img src="<?php echo esc_url($logo_image); ?>" alt="<?php echo esc_attr($logo_text); ?>"
						title="<?php echo esc_attr($logo_text); ?>" class="normal-logo" />
				<?php endif; ?>

				<!-- Mobile logo -->
				<?php if ($logo_image_mobile): ?>
					<img src="<?php echo esc_url($logo_image_mobile); ?>" alt="<?php echo esc_attr($logo_text); ?>"
						title="<?php echo esc_attr($logo_text); ?>" class="normal-logo mobile-logo" />
				<?php endif; ?>

				<!-- Sticky logo -->
				<?php if ($logo_image_sticky): ?>
					<img src="<?php echo esc_url($logo_image_sticky); ?>" alt="<?php echo esc_attr($logo_text); ?>"
						title="<?php echo esc_attr($logo_text); ?>" class="normal-logo sticky-logo" />
				<?php endif; ?>
			...
			<form method="get" id="searchform<?php echo esc_attr($rand) ?>" action="<?php echo esc_url(home_url('/')) ?>">
				<?php if ($enable_category): ?>
					<select class="select-category" name="term">
						<?php echo boxshop_search_by_category_get_option_html($taxonomy, 0, 0); ?>
					</select>
				<?php endif; ?>
				<div class="search-content">
					<input type="text" value="<?php echo get_search_query() ?>" name="s" id="s<?php echo esc_attr($rand) ?>"
						placeholder="<?php echo esc_attr($placeholder_text) ?>" autocomplete="off" />
					<input type="submit" title="<?php esc_attr_e('Search', 'boxshop') ?>"
						id="searchsubmit<?php echo esc_attr($rand) ?>" value="<?php esc_attr_e('Search', 'boxshop') ?>" />
...
if (!function_exists('boxshop_woocommerce_get_catalog_ordering_args_filter')) {
	function boxshop_woocommerce_get_catalog_ordering_args_filter($args)
	{
		global $boxshop_theme_options;
		if (
			is_search() && !isset($_GET['orderby']) && get_option('woocommerce_default_catalog_orderby') == 'menu_order'
...
function boxshop_custom_sidebar_form()
{
	...
	<form action="<?php echo admin_url('widgets.php'); ?>" method="post" id="ts-form-add-sidebar">
		<input type="text" name="sidebar_name" id="sidebar_name"
			placeholder="<?php esc_attr_e('Custom Sidebar Name', 'boxshop'); ?>" />
		<input type="hidden" id="ts_custom_sidebar_nonce" value="<?php echo wp_create_nonce('ts-custom-sidebar'); ?>" />
		<button class="button-primary" id="ts-add-sidebar">
			<?php esc_html_e('Add Sidebar', 'boxshop'); ?>
		</button>
	</form>
	<?php
}
...
function boxshop_register_required_plugins()
{
	...
	$plugins = array(
		...
		array(
			...
			'slug' => 'contact-form-7'
			,
			'source' => 'https://downloads.wordpress.org/plugin/contact-form-7.5.8.1.zip'
			,
			'required' => false
			,
			'version' => '5.8.1'
