...
function boxshop_template_loop_product_label(){
	...
	if( $product->is_in_stock() ){
		/* New label */
		if( $boxshop_theme_options['ts_product_show_new_label'] ){
			...
			if( $num_day <= $num_day_setting ){
				echo '<span class="new">'.esc_html($boxshop_theme_options['ts_product_new_label_text']).'</span>';
			}
		}
		...
		if( $product->is_on_sale() ){
			if( $boxshop_theme_options['ts_show_sale_label_as'] != 'text' ){
				...
				echo '<span class="onsale">'.esc_html($boxshop_theme_options['ts_product_sale_label_text']).'</span>';
			}
		}
		
		/* Hot label */
		if( $product->is_featured() ){
			echo '<span class="featured">'.esc_html($boxshop_theme_options['ts_product_feature_label_text']).'</span>';
		}
	}
	else{ /* Out of stock */
		echo '<span class="out-of-stock">'.esc_html($boxshop_theme_options['ts_product_out_of_stock_label_text']).'</span>';
	}
	?>
	</div>
	<?php
...
if( class_exists('YITH_WCWL') ){
	...
	add_filter('yith_wcwl_browse_wishlist_label', 'boxshop_yith_wcwl_browse_wishlist_label', 10, 2);
	function boxshop_yith_wcwl_browse_wishlist_label( $text = '', $product_id = 0 ){
		if( $product_id ){
...
if( class_exists('YITH_Woocompare') ){
	add_action('init', 'boxshop_yith_compare_handle', 30);
	function boxshop_yith_compare_handle(){
